...
ButtonMappingFactory::CreateButtonMappingFromSDLInput(uint8_t portIndex, CONTROLLERBUTTONS_T bitmask) {
    ...
    for (auto [instanceId, gamepad] :
        ...
        // This catches extra buttons (e.g. back paddles on 8BitDo controllers) that aren't
        // in the standard game-controller mapping on some platforms.
        int32_t rawBtn = FindFirstUnmappedRawJoystickButton(gamepad);
        if (rawBtn >= 0) {
            mapping = std::make_shared<SDLButtonToButtonMapping>(portIndex, bitmask,
                                                                 MAKE_RAW_JOYSTICK_BUTTON(rawBtn));
        }
    }
